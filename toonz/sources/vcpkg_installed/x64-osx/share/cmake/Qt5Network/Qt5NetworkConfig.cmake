...
set(Qt5Network_VERSION_STRING 5.15.9)
...
function(_qt5_Network_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Network_install_prefix}/debug/lib/")
    else()
...
macro(_populate_Network_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Network_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Network static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()
    ...
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::Network static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Network)
    ...
    set(Qt5Network_PRIVATE_INCLUDE_DIRS
        "${_qt5Network_install_prefix}/include/qt5/QtNetwork/5.15.9"
        "${_qt5Network_install_prefix}/include/qt5/QtNetwork/5.15.9/QtNetwork"
    )
    ...
    foreach(_module_dep ${_Qt5Network_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Network_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Network_install_prefix}/debug/lib/libQt5Network_debug.prl")
        _qt5_Network_process_prl_file(
            "${_qt5Network_install_prefix}/debug/lib/libQt5Network_debug.prl" DEBUG
            _Qt5Network_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Network_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Network_install_prefix}/lib/libQt5Network.prl")
        _qt5_Network_process_prl_file(
    ...
    set_property(TARGET Qt5::Network
                 ...
                 corewlan;openssl-linked;openssl;system-proxies)
    set_property(TARGET Qt5::Network
                 PROPERTY QT_DISABLED_PRIVATE_FEATURES
                 libproxy;linux-netlink;netlistmgr)
    ...
    if (EXISTS
        "${_qt5Network_install_prefix}/debug/lib/libQt5Network_debug.a"
      AND EXISTS
        "${_qt5Network_install_prefix}/debug/lib/" )
        _populate_Network_target_properties(DEBUG "libQt5Network_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Network_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Network_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
