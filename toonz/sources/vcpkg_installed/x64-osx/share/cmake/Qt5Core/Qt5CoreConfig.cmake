...
set(Qt5Core_VERSION_STRING 5.15.9)
...
function(_qt5_Core_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Core_install_prefix}/debug/lib/")
    else()
...
macro(_populate_Core_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Core_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Core static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()
    ...
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::Core static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Core)
    ...
    set(Qt5Core_PRIVATE_INCLUDE_DIRS
        "${_qt5Core_install_prefix}/include/qt5/QtCore/5.15.9"
        "${_qt5Core_install_prefix}/include/qt5/QtCore/5.15.9/QtCore"
    )
    ...
    foreach(_module_dep ${_Qt5Core_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Core_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Core_install_prefix}/debug/lib/libQt5Core_debug.prl")
        _qt5_Core_process_prl_file(
            "${_qt5Core_install_prefix}/debug/lib/libQt5Core_debug.prl" DEBUG
            _Qt5Core_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Core_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Core_install_prefix}/lib/libQt5Core.prl")
        _qt5_Core_process_prl_file(
    ...
    set_property(TARGET Qt5::Core
                 ...
                 clock-gettime;datetimeparser;doubleconversion;futimens;hijricalendar;mimetype-database;system-pcre2;poll_poll;sha3-fast;system-doubleconversion)
    set_property(TARGET Qt5::Core
                 PROPERTY QT_DISABLED_PRIVATE_FEATURES
                 etw;futimes;getauxval;getentropy;glib;glibc;gnu-libiconv;icu;posix-libiconv;iconv;inotify;journald;linkat;lttng;poll_ppoll;poll_pollts;poll_select;renameat2;slog2;statx;syslog)
    ...
    if (EXISTS
        "${_qt5Core_install_prefix}/debug/lib/libQt5Core_debug.a"
      AND EXISTS
        "${_qt5Core_install_prefix}/debug/lib/" )
        _populate_Core_target_properties(DEBUG "libQt5Core_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Core_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Core_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
