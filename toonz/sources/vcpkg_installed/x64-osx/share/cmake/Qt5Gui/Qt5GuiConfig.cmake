...
set(Qt5Gui_VERSION_STRING 5.15.9)
...
function(_qt5_Gui_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Gui_install_prefix}/debug/lib/")
    else()
...
macro(_populate_Gui_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Gui_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Gui static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()
    ...
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::Gui static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Gui)
    ...
    set(Qt5Gui_PRIVATE_INCLUDE_DIRS
        "${_qt5Gui_install_prefix}/include/qt5/QtGui/5.15.9"
        "${_qt5Gui_install_prefix}/include/qt5/QtGui/5.15.9/QtGui"
    )
    ...
    foreach(_module_dep ${_Qt5Gui_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Gui_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Gui_install_prefix}/debug/lib/libQt5Gui_debug.prl")
        _qt5_Gui_process_prl_file(
            "${_qt5Gui_install_prefix}/debug/lib/libQt5Gui_debug.prl" DEBUG
            _Qt5Gui_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Gui_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Gui_install_prefix}/lib/libQt5Gui.prl")
        _qt5_Gui_process_prl_file(
    ...
    set_property(TARGET Qt5::Gui PROPERTY INTERFACE_QT_DISABLED_FEATURES angle;combined-angle-lib;dynamicgl;opengles2;opengles3;opengles31;opengles32;openvg;system-textmarkdownreader;vulkan)
    ...
    set_property(TARGET Qt5::Gui
                 ...
                 angle;combined-angle-lib;dynamicgl;opengles2;opengles3;opengles31;opengles32;openvg;system-textmarkdownreader;vulkan)
    set_property(TARGET Qt5::Gui
                 PROPERTY QT_ENABLED_PRIVATE_FEATURES
                 freetype;system-freetype;fontconfig;gif;harfbuzz;ico;imageio-text-loading;jpeg;multiprocess;png;raster-64bit;system-harfbuzz;system-jpeg;system-png;tuiotouch;vkgen)
    set_property(TARGET Qt5::Gui
    ...
    if (EXISTS
        "${_qt5Gui_install_prefix}/debug/lib/libQt5Gui_debug.a"
      AND EXISTS
        "${_qt5Gui_install_prefix}/debug/lib/" )
        _populate_Gui_target_properties(DEBUG "libQt5Gui_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Gui_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Gui_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
