...
set(Qt5MultimediaWidgets_VERSION_STRING 5.15.9)
...
function(_qt5_MultimediaWidgets_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5MultimediaWidgets_install_prefix}/debug/lib/")
    else()
        set(_qt5_install_libs "${_qt5MultimediaWidgets_install_prefix}/lib/")
    endif()
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5MultimediaWidgets_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5MultimediaWidgets_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5MultimediaWidgets_${Configuration}_${_lib}_PATH)
                    if(_Qt5MultimediaWidgets_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5MultimediaWidgets_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_MultimediaWidgets_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5MultimediaWidgets_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    set(_static_deps
        ${_Qt5MultimediaWidgets_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::MultimediaWidgets static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()

    set(_static_link_flags "${_Qt5MultimediaWidgets_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::MultimediaWidgets static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::MultimediaWidgets)
    ...
    set(Qt5MultimediaWidgets_PRIVATE_INCLUDE_DIRS
        "${_qt5MultimediaWidgets_install_prefix}/include/qt5/QtMultimediaWidgets/5.15.9"
        "${_qt5MultimediaWidgets_install_prefix}/include/qt5/QtMultimediaWidgets/5.15.9/QtMultimediaWidgets"
    )
    ...
    foreach(_module_dep ${_Qt5MultimediaWidgets_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5MultimediaWidgets_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5MultimediaWidgets_install_prefix}/debug/lib/libQt5MultimediaWidgets_debug.prl")
        _qt5_MultimediaWidgets_process_prl_file(
            "${_qt5MultimediaWidgets_install_prefix}/debug/lib/libQt5MultimediaWidgets_debug.prl" DEBUG
            _Qt5MultimediaWidgets_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5MultimediaWidgets_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5MultimediaWidgets_install_prefix}/lib/libQt5MultimediaWidgets.prl")
        _qt5_MultimediaWidgets_process_prl_file(
            "${_qt5MultimediaWidgets_install_prefix}/lib/libQt5MultimediaWidgets.prl" RELEASE
            _Qt5MultimediaWidgets_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5MultimediaWidgets_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    endif()
    ...
    if (EXISTS
        "${_qt5MultimediaWidgets_install_prefix}/debug/lib/libQt5MultimediaWidgets_debug.a"
      AND EXISTS
        "${_qt5MultimediaWidgets_install_prefix}/debug/lib/" )
        _populate_MultimediaWidgets_target_properties(DEBUG "libQt5MultimediaWidgets_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_MultimediaWidgets_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5MultimediaWidgets_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
