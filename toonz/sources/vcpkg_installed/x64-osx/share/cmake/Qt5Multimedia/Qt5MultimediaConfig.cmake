...
set(Qt5Multimedia_VERSION_STRING 5.15.9)
...
function(_qt5_Multimedia_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Multimedia_install_prefix}/debug/lib/")
    else()
        set(_qt5_install_libs "${_qt5Multimedia_install_prefix}/lib/")
    endif()
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5Multimedia_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5Multimedia_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5Multimedia_${Configuration}_${_lib}_PATH)
                    if(_Qt5Multimedia_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5Multimedia_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_Multimedia_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Multimedia_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    set(_static_deps
        ${_Qt5Multimedia_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Multimedia static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()

    set(_static_link_flags "${_Qt5Multimedia_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::Multimedia static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Multimedia)
    ...
    set(Qt5Multimedia_PRIVATE_INCLUDE_DIRS
        "${_qt5Multimedia_install_prefix}/include/qt5/QtMultimedia/5.15.9"
        "${_qt5Multimedia_install_prefix}/include/qt5/QtMultimedia/5.15.9/QtMultimedia"
    )
    ...
    foreach(_module_dep ${_Qt5Multimedia_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Multimedia_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Multimedia_install_prefix}/debug/lib/libQt5Multimedia_debug.prl")
        _qt5_Multimedia_process_prl_file(
            "${_qt5Multimedia_install_prefix}/debug/lib/libQt5Multimedia_debug.prl" DEBUG
            _Qt5Multimedia_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Multimedia_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Multimedia_install_prefix}/lib/libQt5Multimedia.prl")
        _qt5_Multimedia_process_prl_file(
            "${_qt5Multimedia_install_prefix}/lib/libQt5Multimedia.prl" RELEASE
            _Qt5Multimedia_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5Multimedia_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    endif()
    ...
    set_property(TARGET Qt5::Multimedia
                 ...
                 alsa;directshow;evr;gpu_vivante;gstreamer_1_0;gstreamer_0_10;gstreamer;gstreamer_app;gstreamer_encodingprofiles;gstreamer_gl;gstreamer_photography;linux_v4l;pulseaudio;resourcepolicy;wasapi;wmf;wmsdk;wshellitem)
    ...
    if (EXISTS
        "${_qt5Multimedia_install_prefix}/debug/lib/libQt5Multimedia_debug.a"
      AND EXISTS
        "${_qt5Multimedia_install_prefix}/debug/lib/" )
        _populate_Multimedia_target_properties(DEBUG "libQt5Multimedia_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Multimedia_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Multimedia_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
