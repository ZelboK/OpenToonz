...
set(Qt5ScriptTools_VERSION_STRING 5.15.9)
...
function(_qt5_ScriptTools_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5ScriptTools_install_prefix}/debug/lib/")
    else()
        set(_qt5_install_libs "${_qt5ScriptTools_install_prefix}/lib/")
    endif()
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5ScriptTools_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5ScriptTools_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5ScriptTools_${Configuration}_${_lib}_PATH)
                    if(_Qt5ScriptTools_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5ScriptTools_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_ScriptTools_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5ScriptTools_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    set(_static_deps
        ${_Qt5ScriptTools_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::ScriptTools static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()

    set(_static_link_flags "${_Qt5ScriptTools_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::ScriptTools APPEND PROPERTY INTERFACE_LINK_OPTIONS
            ...
            #message(STATUS "Target Qt5::ScriptTools static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::ScriptTools)
    ...
    set(_Qt5ScriptTools_OWN_INCLUDE_DIRS "${_qt5ScriptTools_install_prefix}/include/qt5/" "${_qt5ScriptTools_install_prefix}/include/qt5/QtScriptTools")
    set(Qt5ScriptTools_PRIVATE_INCLUDE_DIRS
        "${_qt5ScriptTools_install_prefix}/include/qt5/QtScriptTools/5.15.9"
        "${_qt5ScriptTools_install_prefix}/include/qt5/QtScriptTools/5.15.9/QtScriptTools"
    )
    ...
    foreach(_module_dep ${_Qt5ScriptTools_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5ScriptTools_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5ScriptTools_install_prefix}/debug/lib/libQt5ScriptTools_debug.prl")
        _qt5_ScriptTools_process_prl_file(
            "${_qt5ScriptTools_install_prefix}/debug/lib/libQt5ScriptTools_debug.prl" DEBUG
            _Qt5ScriptTools_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5ScriptTools_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5ScriptTools_install_prefix}/lib/libQt5ScriptTools.prl")
        _qt5_ScriptTools_process_prl_file(
            "${_qt5ScriptTools_install_prefix}/lib/libQt5ScriptTools.prl" RELEASE
            _Qt5ScriptTools_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5ScriptTools_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    endif()
    ...
    _populate_ScriptTools_target_properties(RELEASE "libQt5ScriptTools.a" "" FALSE)

    if (EXISTS
        "${_qt5ScriptTools_install_prefix}/debug/lib/libQt5ScriptTools_debug.a"
      AND EXISTS
        "${_qt5ScriptTools_install_prefix}/debug/lib/" )
        _populate_ScriptTools_target_properties(DEBUG "libQt5ScriptTools_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_ScriptTools_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5ScriptTools_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
