...
set(Qt5Concurrent_VERSION_STRING 5.15.9)
...
function(_qt5_Concurrent_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Concurrent_install_prefix}/debug/lib/")
    else()
...
macro(_populate_Concurrent_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Concurrent_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Concurrent static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()
    ...
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::Concurrent static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Concurrent)
    ...
    foreach(_module_dep ${_Qt5Concurrent_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Concurrent_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Concurrent_install_prefix}/debug/lib/libQt5Concurrent_debug.prl")
        _qt5_Concurrent_process_prl_file(
            "${_qt5Concurrent_install_prefix}/debug/lib/libQt5Concurrent_debug.prl" DEBUG
            _Qt5Concurrent_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Concurrent_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Concurrent_install_prefix}/lib/libQt5Concurrent.prl")
        _qt5_Concurrent_process_prl_file(
    ...
    if (EXISTS
        "${_qt5Concurrent_install_prefix}/debug/lib/libQt5Concurrent_debug.a"
      AND EXISTS
        "${_qt5Concurrent_install_prefix}/debug/lib/" )
        _populate_Concurrent_target_properties(DEBUG "libQt5Concurrent_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Concurrent_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Concurrent_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
