...
set(Qt5MultimediaQuick_VERSION_STRING 5.15.9)
...
function(_qt5_MultimediaQuick_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5MultimediaQuick_install_prefix}/debug/lib/")
    else()
        set(_qt5_install_libs "${_qt5MultimediaQuick_install_prefix}/lib/")
    endif()
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5MultimediaQuick_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5MultimediaQuick_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5MultimediaQuick_${Configuration}_${_lib}_PATH)
                    if(_Qt5MultimediaQuick_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5MultimediaQuick_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_MultimediaQuick_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5MultimediaQuick_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    set(_static_deps
        ${_Qt5MultimediaQuick_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::MultimediaQuick static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()

    set(_static_link_flags "${_Qt5MultimediaQuick_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::MultimediaQuick static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::MultimediaQuick)
    ...
    set(_Qt5MultimediaQuick_OWN_INCLUDE_DIRS "${_qt5MultimediaQuick_install_prefix}/include/qt5/" "${_qt5MultimediaQuick_install_prefix}/include/qt5/QtMultimediaQuick")
    set(Qt5MultimediaQuick_PRIVATE_INCLUDE_DIRS
        "${_qt5MultimediaQuick_install_prefix}/include/qt5/QtMultimediaQuick/5.15.9"
        "${_qt5MultimediaQuick_install_prefix}/include/qt5/QtMultimediaQuick/5.15.9/QtMultimediaQuick"
    )
    ...
    foreach(_module_dep ${_Qt5MultimediaQuick_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5MultimediaQuick_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5MultimediaQuick_install_prefix}/debug/lib/libQt5MultimediaQuick_debug.prl")
        _qt5_MultimediaQuick_process_prl_file(
            "${_qt5MultimediaQuick_install_prefix}/debug/lib/libQt5MultimediaQuick_debug.prl" DEBUG
            _Qt5MultimediaQuick_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5MultimediaQuick_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5MultimediaQuick_install_prefix}/lib/libQt5MultimediaQuick.prl")
        _qt5_MultimediaQuick_process_prl_file(
            "${_qt5MultimediaQuick_install_prefix}/lib/libQt5MultimediaQuick.prl" RELEASE
            _Qt5MultimediaQuick_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5MultimediaQuick_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    endif()

    add_library(Qt5::MultimediaQuick STATIC IMPORTED)
    set_property(TARGET Qt5::MultimediaQuick PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    _populate_MultimediaQuick_target_properties(RELEASE "libQt5MultimediaQuick.a" "" FALSE)

    if (EXISTS
        "${_qt5MultimediaQuick_install_prefix}/debug/lib/libQt5MultimediaQuick_debug.a"
      AND EXISTS
        "${_qt5MultimediaQuick_install_prefix}/debug/lib/" )
        _populate_MultimediaQuick_target_properties(DEBUG "libQt5MultimediaQuick_debug.a" "" FALSE)
    endif()
