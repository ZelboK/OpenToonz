...
set(Qt5SerialPort_VERSION_STRING 5.15.9)
...
function(_qt5_SerialPort_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5SerialPort_install_prefix}/debug/lib/")
    else()
        set(_qt5_install_libs "${_qt5SerialPort_install_prefix}/lib/")
    endif()
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5SerialPort_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5SerialPort_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
...
macro(_populate_SerialPort_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5SerialPort_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::SerialPort static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()

    set(_static_link_flags "${_Qt5SerialPort_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::SerialPort APPEND PROPERTY INTERFACE_LINK_OPTIONS
            ...
            #message(STATUS "Target Qt5::SerialPort static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::SerialPort)
    ...
    set(_Qt5SerialPort_OWN_INCLUDE_DIRS "${_qt5SerialPort_install_prefix}/include/qt5/" "${_qt5SerialPort_install_prefix}/include/qt5/QtSerialPort")
    set(Qt5SerialPort_PRIVATE_INCLUDE_DIRS
        "${_qt5SerialPort_install_prefix}/include/qt5/QtSerialPort/5.15.9"
        "${_qt5SerialPort_install_prefix}/include/qt5/QtSerialPort/5.15.9/QtSerialPort"
    )
    ...
    foreach(_module_dep ${_Qt5SerialPort_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5SerialPort_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5SerialPort_install_prefix}/debug/lib/libQt5SerialPort_debug.prl")
        _qt5_SerialPort_process_prl_file(
            "${_qt5SerialPort_install_prefix}/debug/lib/libQt5SerialPort_debug.prl" DEBUG
            _Qt5SerialPort_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5SerialPort_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5SerialPort_install_prefix}/lib/libQt5SerialPort.prl")
        _qt5_SerialPort_process_prl_file(
            "${_qt5SerialPort_install_prefix}/lib/libQt5SerialPort.prl" RELEASE
    ...
    _populate_SerialPort_target_properties(RELEASE "libQt5SerialPort.a" "" FALSE)

    if (EXISTS
        "${_qt5SerialPort_install_prefix}/debug/lib/libQt5SerialPort_debug.a"
      AND EXISTS
        "${_qt5SerialPort_install_prefix}/debug/lib/" )
        _populate_SerialPort_target_properties(DEBUG "libQt5SerialPort_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_SerialPort_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5SerialPort_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
