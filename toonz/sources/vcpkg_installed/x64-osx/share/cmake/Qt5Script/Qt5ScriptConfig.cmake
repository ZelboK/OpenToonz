...
set(Qt5Script_VERSION_STRING 5.15.9)
...
function(_qt5_Script_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Script_install_prefix}/debug/lib/")
    else()
        set(_qt5_install_libs "${_qt5Script_install_prefix}/lib/")
    endif()
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5Script_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5Script_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
...
macro(_populate_Script_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Script_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Script static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()

    set(_static_link_flags "${_Qt5Script_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::Script APPEND PROPERTY INTERFACE_LINK_OPTIONS
            ...
            #message(STATUS "Target Qt5::Script static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Script)
    ...
    set(Qt5Script_PRIVATE_INCLUDE_DIRS
        "${_qt5Script_install_prefix}/include/qt5/QtScript/5.15.9"
        "${_qt5Script_install_prefix}/include/qt5/QtScript/5.15.9/QtScript"
    )
    ...
    foreach(_module_dep ${_Qt5Script_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Script_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Script_install_prefix}/debug/lib/libQt5Script_debug.prl")
        _qt5_Script_process_prl_file(
            "${_qt5Script_install_prefix}/debug/lib/libQt5Script_debug.prl" DEBUG
            _Qt5Script_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Script_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Script_install_prefix}/lib/libQt5Script.prl")
        _qt5_Script_process_prl_file(
            "${_qt5Script_install_prefix}/lib/libQt5Script.prl" RELEASE
    ...
    _populate_Script_target_properties(RELEASE "libQt5Script.a" "" FALSE)

    if (EXISTS
        "${_qt5Script_install_prefix}/debug/lib/libQt5Script_debug.a"
      AND EXISTS
        "${_qt5Script_install_prefix}/debug/lib/" )
        _populate_Script_target_properties(DEBUG "libQt5Script_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Script_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Script_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
