...
set(Qt5Sql_VERSION_STRING 5.15.9)
...
function(_qt5_Sql_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5Sql_install_prefix}/debug/lib/")
    else()
...
macro(_populate_Sql_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5Sql_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::Sql static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()
    ...
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::Sql static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::Sql)
    ...
    set(Qt5Sql_PRIVATE_INCLUDE_DIRS
        "${_qt5Sql_install_prefix}/include/qt5/QtSql/5.15.9"
        "${_qt5Sql_install_prefix}/include/qt5/QtSql/5.15.9/QtSql"
    )
    ...
    foreach(_module_dep ${_Qt5Sql_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.9 ${_Qt5Sql_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5Sql_install_prefix}/debug/lib/libQt5Sql_debug.prl")
        _qt5_Sql_process_prl_file(
            "${_qt5Sql_install_prefix}/debug/lib/libQt5Sql_debug.prl" DEBUG
            _Qt5Sql_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5Sql_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5Sql_install_prefix}/lib/libQt5Sql.prl")
        _qt5_Sql_process_prl_file(
    ...
    if (EXISTS
        "${_qt5Sql_install_prefix}/debug/lib/libQt5Sql_debug.a"
      AND EXISTS
        "${_qt5Sql_install_prefix}/debug/lib/" )
        _populate_Sql_target_properties(DEBUG "libQt5Sql_debug.a" "" FALSE)
    endif()
    ...
    macro(_populate_Sql_plugin_properties Plugin Configuration PLUGIN_LOCATION
        ...
        if("${Configuration}" STREQUAL "DEBUG")
            set(imported_location "${_qt5Sql_install_prefix}/debug/plugins/${PLUGIN_LOCATION}")
        else()
